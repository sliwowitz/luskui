...
* The files in `/opt/luskui` provide a **lightweight web UI** (SDK-powered) that mimics a "Cloud AI" flow:
...
* **LuskUI** (this app): `/opt/luskui` — the UI is meant to be installed there inside the container.
...
* **Project root** – `/opt/luskui` inside the production image (or `/workspace/codex-in-podman` when iterating locally)
  ...
  * `lib/` — runtime helpers used by the server:
    ...
    * `logging.js` — appends structured lines to `/opt/luskui/luskui.log` (configurable via `LUSKUI_LOG`).
...
The Express server is configured to serve static assets from `$LUSKUI_DIR/static` in production, but during development the
...
The `uc.luskui.sdk.containerfile` image (see sample snippet below) wires up the runtime like so:

1. Verifies SSH material in `/home/dev/.ssh`, copies it to `/root/.ssh`, and warms `github.com` host keys.
2. Clones (or hard-resets) `LUSKUI_REPO` into `LUSKUI_DIR` – defaults to `/opt/luskui`.
3. Runs `npm ci --omit=dev` (or `npm install --omit=dev`) inside `LUSKUI_DIR`.
...
ENV REPO_ROOT="/workspace/ultimate-container" \
    ...
    LUSKUI_REPO="git@github.com:sliwowitz/luskui.git"
```
...
`luskui-dist.tar.gz` asset in the `latest` GitHub release. The tarball contains:
...
* **Logging**
  * Every run/action is logged via `lib/logging.js` to stdout and (when available) `/opt/luskui/luskui.log`.
...
| `LUSKUI_LOG` | Override the log file path (defaults to `/opt/luskui/luskui.log`). |
| `LUSKUI_BACKEND` | Select the active backend (`codex`, `claude`, or `mistral`). |
| `LUSKUI_MODEL`, `LUSKUI_EFFORT` | Force default model/effort without editing `~/.codex/config.toml`. |
| `LUSKUI_CLAUDE_MODEL` | Override the default Claude model when using the Claude backend. |
| `LUSKUI_MISTRAL_MODEL` | Override the default Mistral model when using the Mistral backend. |
| `LUSKUI_SKIP_GIT_CHECK` | Skip verifying that `REPO_ROOT` contains a Git repo (enabled automatically if `.git` missing). |
| `LUSKUI_MODEL_CACHE_MS` | TTL for cached model lists (defaults to 5 minutes). |
| `LUSKUI_MODEL_FETCH_TIMEOUT_MS` | Timeout for the model list request (defaults to 5 seconds). |

The app reads OpenAI API keys from `OPENAI_API_KEY`, `CODEX_API_KEY`, or `~/.codex/auth.json`. For Claude, set
`LUSKUI_CLAUDE_API_KEY`, `ANTHROPIC_API_KEY`, or `CLAUDE_API_KEY`. For Mistral, set `LUSKUI_MISTRAL_API_KEY`
or `MISTRAL_API_KEY`.
