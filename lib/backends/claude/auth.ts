...
function getClaudeOauthToken(): string | null {
  return (
    process.env.CODEXUI_CLAUDE_OAUTH_ACCESS_TOKEN || process.env.CLAUDE_OAUTH_ACCESS_TOKEN || null
  );
}
...
function shouldReuseCachedKey(oauthToken: string): boolean {
  ...
  const ttlMs = Number(process.env.CODEXUI_CLAUDE_OAUTH_CACHE_MS || 5 * 60 * 1000);
