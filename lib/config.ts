...
export const REPO_ROOT_ABS: string = path.resolve(REPO_ROOT);
export const LOG_PATH: string = process.env.CODEXUI_LOG || path.join("/var/log", "codexui.log");

const HOME_DIR = os.homedir?.() ?? process.env.HOME ?? "";
export const CODEX_CONFIG_PATH: string = process.env.CODEX_CONFIG || path.join(HOME_DIR, ".codex", "config.toml");
export const CODEX_AUTH_PATH: string = process.env.CODEX_AUTH || path.join(HOME_DIR, ".codex", "auth.json");
...
export const DEFAULT_EFFORT: string | null = process.env.CODEXUI_EFFORT || DEFAULT_EFFORT_FROM_CONFIG || null;
...
export function resolveRepoPath(relPath = ""): ResolvedPath {
